...
from Advisor.Random import Random
...
class BaseOptimizer:
    def __init__(self, config_space: ConfigurationSpace, eval_func, 
                 ...
                 space_history = None, cprs_strategy='none', cprs_k=50,
                 backup_flag=False, save_dir='./results',
                 seed=42, rand_prob=0.15, rand_mode='ran', config_modifier = None, **kwargs):

        assert method_id in ['RS', 'SMAC', 'GP', 'GPF', 'MFSE_SMAC', 'MFSE_GP', 'BOHB_GP', 'BOHB_SMAC', 'FlexHB_SMAC', 'TAS-BO']
        ...
        assert cprs_strategy in ['REMBO', 'ALEBO', 'BAxUS', 'Dropout-VS', 'MCTS-VS', 'Gr-VS', 'ours', 'Add-GP-UCB', 'PP-GP-UCB', 'RPP-GP-UCB', 'none']
        ...
        self.task_id = '%s_%s%s__%sW%sT%sE%sC%s__s%d' % (task_id, target, task_str, self.method_id,
                                                         ws_str, tl_str, ep_strategy, cprs_strategy, seed)
        self.target = target
        ...
        """
            ...
            ws_strategy: none, best_cos, best_euc, best_rover, rgpe_rover
        ...
        if method_id == 'RS':
            self.advisor = Random(config_space, meta_feature=meta_feature,
        ...
        if method_id in ['SMAC', 'GP'] or 'BOHB' in method_id:
            self.advisor = BO(config_space, source_hpo_data=source_hpo_data,
                              ...
                              _logger_kwargs=self._logger_kwargs, **kwargs)
    ...
    def save_info(self, interval=5):
        ...
        if self.iter_id == self.iter_num:
            ...
            if self.cprs_strategy == 'REMBO':
                self.advisor.origin_history.save_json(self.result_path + '.origin')
            if self.backup_flag:
                self.record_task()
                with open(self.ts_backup_file, 'wb') as ts:
                    pkl.dump(self.ts_recorder, ts)

        elif self.iter_id % interval == 0:
            self.advisor.history.save_json(self.result_path)
            if self.cprs_strategy == 'REMBO':
                self.advisor.origin_history.save_json(self.result_path + '.origin')
