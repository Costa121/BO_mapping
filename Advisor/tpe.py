...
class TPEAdvisor(BaseAdvisor):
    def __init__(
            self,
            config_space: ConfigurationSpace, eval_func, iter_num=10, save_dir='./results',
        ...
        super().__init__(config_space, eval_func, iter_num, task_id=task_id)
    ...
    def update(self, config, perf, qps, context):
        super(TPEAdvisor, self).update(config, perf, qps, context)
    ...
    def sample(self):
        ...
        try:
            ...
            if best_vector is None:
                ...
                config = self.sample_random_configs(1)[0]
            else:
                logger.debug(
    ...
    def fit_kde_models(self):
        num_config_successful = len(self.history)
        ...
        train_configs = self.history.get_config_array()
        train_losses = self.history.get_objectives().reshape(-1)
