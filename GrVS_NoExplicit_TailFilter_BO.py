"""
GrVS-NoExplicit-TailFilter-BO: No-Explicit + TailFilter + 方向符号

核心思想:
  1. 使用 Gr-VS 的 ECI 评分计算变量重要性 rho
  2. **完全不使用 top-k 单轴**，所有维度都轮询映射到 d 个轴
  3. TailFilter: 只保留重要性最高的 Top-M 个维度 (M = d_embed)
  4. 方向符号: 使用 sign(x_{j,max} - x_j*) 确定方向，避免随机符号的不稳定性
  5. 每列 L2 归一化保证尺度统一

与原版 GrVS-Mapping-BO 的区别:
  - 原版: top-k 显式轴 + 轮询分配 + 随机符号
  - 本版: 无显式轴 + 全部轮询 + TailFilter + 方向符号
"""
...
class GrVSNoExplicitTailFilterBO:
    """
    GrVS-NoExplicit-TailFilter-BO 实现
    
    特点:
    - 无显式轴 (k_explicit = 0)
    - TailFilter: 只保留重要性最高的 M 个维度参与映射
    - 方向符号: sign(x_{j,max} - x_j*) 替代随机符号
    - 轮询映射: 所有保留的维度轮询分配到 d 个轴
    """
    ...
    def __init__(
        ...
        tail_filter_ratio: float = 1.5,  # 保留 M = d * ratio 个维度
        eps: float = 1e-12,
    ):
        """
        Args:
            ...
            tail_filter_ratio: TailFilter 保留的维度数 M = min(D, int(d * ratio))
            eps: 数值稳定小常数
        """
        self.f = f
        self.D = int(D)
        ...
        self.active_dims = None      # 保留的活跃维度索引
    ...
    def _compute_eci_scores(self):
        ...
        print(f"\n  📊 [GrVS-NoExplicit] ECI 评分完成:")
        print(f"     Top-5 维度: {top_5.tolist()}")
        print(f"     对应 rho: {[f'{self.rho[i]:.4f}' for i in top_5]}")
    
    # =========================
    # 嵌入矩阵构建 (No-Explicit + TailFilter + 方向符号)
    # =========================
    def _build_embedding_matrix(self):
        """
        No-Explicit + TailFilter + 方向符号
        
        步骤:
        1. TailFilter: 只保留重要性最高的 M 个维度
        2. 方向符号: sign(x_{j,max} - x_j*) 确定方向
        3. 轮询分配: 保留的维度按 rho 加权轮询分配到 d 个轴
        4. 每列 L2 归一化
        """
        D, d = self.D, self.d
        rho = self.rho.copy()
        ...
        # 1. TailFilter: 只保留 Top-M 个维度
        if self.tail_filter:
            M = min(D, max(d, int(d * self.tail_filter_ratio)))
        else:
            M = D
        
        sorted_idx = np.argsort(-rho)  # 按 rho 从大到小排序
        active_dims = sorted_idx[:M]   # 保留重要性最高的 M 个维度
        self.active_dims = active_dims.tolist()
        
        # 2. 计算方向符号: sign(x_{j,max} - x_j*)
        direction = xj_max - x_star
        sgn = np.sign(direction)
        # 防止符号为零（当 x_{j,max} == x_j* 时）
        sgn[sgn == 0] = 1.0
        
        # 3. 初始化嵌入矩阵 A
        A = np.zeros((D, d), dtype=float)
        
        # 4. 轮询分配：所有活跃维度轮询映射到 d 个轴
        # 按重要性排序后的活跃维度
        active_rho = rho[active_dims]
        
        # 分组：轮询分配到 d 个轴
        groups = [[] for _ in range(d)]
        for t_idx, dim in enumerate(active_dims):
            col = t_idx % d
            groups[col].append(dim)
        
        # 对每组内的维度按 rho 加权填充 A
        for col, dims in enumerate(groups):
            if len(dims) == 0:
                continue
            
            # 组内 rho 归一化
            group_rho = rho[dims]
            group_rho_sum = float(np.sum(group_rho) + self.eps)
        ...
        # 打印调试信息
        print(f"\n  🔧 [NoExplicit-TailFilter] 嵌入矩阵更新 (总样本数={len(self.X_history)}):")
        print(f"     TailFilter: 保留 {len(self.active_dims)}/{D} 个维度")
        print(f"     活跃维度 (前5): {self.active_dims[:min(5, len(self.active_dims))]}...")
    ...
    def update_importance_and_embedding(self):
        ...
        print(f"\n{'='*60}")
        print(f"🔄 [NoExplicit-TailFilter] 段 {self._segment_count}: 更新重要性和嵌入矩阵")
    ...
    def _run_segment_bo(self, n_runs: int, segment_id: int):
        ...
        opt = Optimizer(
            ...
            task_id=f'noexplicit_seg{segment_id}_D{self.D}_d{self.d}'
        )
    ...
    def run_optimization(self, init_n=20, max_iters=100):
        """运行优化（方案 A4: 段间更新 A）"""
        print(f"\n{'='*80}")
        print(f"GrVS-NoExplicit-TailFilter-BO (No-Explicit + TailFilter + 方向符号)")
        print(f"D={self.D}, d={self.d}, seed={self.seed}")
...
def run_grvs_noexplicit_tailfilter_bo(
    f, D, d,
    ...
    seed=42,
    save_results=True,
    task_id_prefix="grvs_noexplicit_tailfilter"
):
    """
    GrVS-NoExplicit-TailFilter-BO 便捷接口
    ...
    Args:
        ...
        rho_momentum: ρ 平滑系数
        tail_filter: 是否启用 TailFilter
        tail_filter_ratio: 保留维度数 M = min(D, int(d * ratio))
        seed: 随机种子
        save_results: 是否保存结果
        task_id_prefix: 任务 ID 前缀
    """
    optimizer = GrVSNoExplicitTailFilterBO(
        f=f,
        D=D,
        d=d,
    ...
    if save_results:
        results_dir = Path("results_grvs_noexplicit_tailfilter")
        ...
        result_data = {
            ...
            'active_dims': optimizer.active_dims,
            ...
            'config': {
                'method': 'GrVS-NoExplicit-TailFilter-BO',
                'D': int(D),
                'd': int(d),
                'k_explicit': 0,
        ...
        save_path = results_dir / f"{task_id_prefix}_D{D}_d{d}_s{seed}.json"
